...
const HOME_TAB: TabItem = { title: '首页', path: '/dashboard', name: 'Dashboard', closable: false }
...
export function useTabs() {
    ...
    const closeOthers = (currentPath: string) => {
        tabs.value = tabs.value.filter((tab: TabItem) => !tab.closable || tab.path === currentPath)
    ...
    const closeLeft = (currentPath: string) => {
        const index = tabs.value.findIndex((tab: TabItem) => tab.path === currentPath)
        tabs.value = tabs.value.filter((tab: TabItem, i: number) => i >= index || !tab.closable)
        activeKey.value = currentPath
        router.push(currentPath)
    }
