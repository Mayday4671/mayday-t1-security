<template>
  <div class="page-container">
    <a-card title="角色管理">
      ...
      <a-table :columns="columns" :dataSource="roleList" :loading="loading" rowKey="id">
    ...
    <!-- 新增/编辑弹窗 -->
    <a-modal v-model:open="formVisible" :title="formTitle" @ok="handleSubmit" :confirmLoading="submitting">
      <a-form :model="formData" layout="vertical">
        <a-form-item label="角色标识" v-if="!formData.id">
          <a-input v-model:value="formData.roleKey" placeholder="如：admin, manager" />
        </a-form-item>
        <a-form-item label="角色名称">
          <a-input v-model:value="formData.roleName" placeholder="如：超级管理员" />
        </a-form-item>
        <a-form-item label="数据权限">
          <a-select v-model:value="formData.dataScope" placeholder="选择数据权限范围">
    ...
    <!-- 分配权限弹窗 -->
    <a-modal v-model:open="menuVisible" title="分配菜单权限" @ok="handleMenuSubmit" :confirmLoading="submitting" width="600px">
      <a-tree
...
const getDataScopeColor = (scope: string) => {
  const map: Record<string, string> = {
    '1': 'red', '2': 'purple', '3': 'blue', '4': 'orange', '5': 'green'
  }
...
const handleEdit = async (record: any) => {
  try {
    const res = await request.get(`/system/role/${record.id}`)
    formData.value = { ...res }
    formVisible.value = true
  } catch (e) {
